...
| `DOSSIER-005` | M13A Q&A 2.1 states the twelve-evaluable-subject floor for PIVOTAL bioequivalence studies. `minimums.py` carries the figures and the word 'evaluable' but not the word 'pivotal', so a caller running a pilot relative bioavailability study is returned a floor the document does not place on it. | scope_limitation | open |
...
Open: `VAL-FDA-APPENDIX-C-002`, `DOSSIER-001`, `DOSSIER-002`, `DOSSIER-005`, `DOSSIER-003`.
